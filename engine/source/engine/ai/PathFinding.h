...
namespace gswy
{
	...
	public:
		explicit PathFinding(int x, int y)
			...
			COL(y)
		{}
		bool Search(Grid& grid, const ivec2& src, const ivec2& dest)
		{
	...
	private:
		...
		bool aStarSearch(Grid& grid, Pair src, Pair dest)
		{
			...
			bool** closedList = (bool**)MemoryManager::Allocate(ROW * sizeof(bool*));
			for (int i = 0; i < ROW; ++i)
			{
			...
			//of that cell 
			cell** cellDetails = (cell**)MemoryManager::Allocate(ROW * sizeof(cell*));
			for (int i = 0; i < ROW; ++i)
			{
